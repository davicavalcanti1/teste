...
import { InspectionOverview } from "@/hooks/useInspections";
import { format, parseISO, startOfDay } from "date-fns";
import { ptBR } from "date-fns/locale";

interface InspectionChartsProps {
    data: InspectionOverview[];
    type: "geral" | "dispenser" | "banheiro" | "ar_condicionado";
}
...
export function InspectionCharts({ data, type }: InspectionChartsProps) {
    ...
    const problemData = useMemo(() => {
        ...
        data.forEach(item => {
            ...
            let key = item.resumo || "Não especificado";
            ...
            <Card>
                <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-medium">
                        {type === 'ar_condicionado' ? "Por Origem (Equipe)" : "Tipos de Problema"}
                    </CardTitle>
                </CardHeader>
                <CardContent>
